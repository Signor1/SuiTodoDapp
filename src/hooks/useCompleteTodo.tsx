...
const useCompleteTodo = () => {
    ...
    return useCallback(async (id: string) => {
        ...
        const completed = true; // Assuming you want to mark the todo as completed

        const transaction = new Transaction();
        transaction.moveCall({
            target: `${import.meta.env.VITE_PUBLIC_PACKAGE_ID}::todolist::todo_completed`,
            arguments: [
                transaction.object(todoListId),
                transaction.pure(bcs.u64().serialize(id.toString()).toBytes()),
                transaction.pure(bcs.bool().serialize(completed))
            ]
        });
        ...
        try {
            await signAndExecuteTransaction({
                ...
                onSuccess: (txResult) => {
                    toast.success(`Todo completed! Digest: ${txResult.digest}`);
